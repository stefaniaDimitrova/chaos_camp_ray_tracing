...
CRTRay generatedRay (int x, int y)
{
...
void generateTriangle(CRTTriangle &triangle, Picture& result, int red , int green, int blue)
{
    ...
    for (size_t y = 0; y < imageHeight; y++)
    {
        for (size_t x = 0; x < imageWidth; x++)
        {
            ...
            CRTRay cameraRay = generatedRay(x, y);
            ...
            if (normal.dotProduct(cameraRay.direction) != 0)
            {
                ...
                float rpDist = triangle.v0().dotProduct(normal);
...
int main()
{
    ...
    // CRTTriangle triangle(CRTVector(-1.75, -1.75, -3), 
    //             CRTVector(1.75,-1.75,-3), 
    //             CRTVector(0,1.75,-3));

    // CRTTriangle triangle2(CRTVector(-1,-1,-3 ), 
    //             CRTVector(1,-1,-3), 
    //             CRTVector(0,1,-3));
    ...
    generateTriangle(leftFace,result, 255,0,255);
    generateTriangle(rightFace, result, 255,0,0);
